...
const adminComponent = {
    ...
    class AdminController {
        static get $inject() {
            return ['$log', '$timeout', '$scope', 'FormApi', 'FormApiLocal', 'adminService', '$state'];
        }
        constructor($log, $timeout, $scope, FormApi, FormApiLocal, adminService, $state) {
            this.$log = $log;
        ...
        generateFile(selectedAbs) {
            for (let i=0; i<selectedAbs.length; i++) {
                let label = document.querySelector(
                                '#label_' + selectedAbs[i] + ' .js-order_label'
                            ).innerHTML.trim();
                if (label.length === 1) {
                    label = "0" + label;
                }
